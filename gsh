...
ssh_try_connect() {
  ...
  if grep -q "REMOTE HOST IDENTIFICATION HAS CHANGED" <<<"$err" || \
    ...
    echo "âš ï¸  SSH Host Key changed for: $alias"
    ...
    if confirm_yn "Remove old known_hosts entries and retry?"; then
      ...
      echo "ðŸ§¹ Cleaning known_hosts entries..."
      ...
      echo "âœ… Removed old keys. Retrying connection..."
      # retry once
      exec ssh "$alias"
    else
      echo "âŒ Not changed. Connection aborted (to keep you safe)."
      return 255
    fi
  fi

  # Not a hostkey error â†’ show original stderr
  echo "$err" >&2
  return "$rc"
}
...
cmd_add() {
  ...
  if [[ -z "$name" || -z "$ip" ]]; then
    echo "âŒ Name and IP/Host are required."
    exit 1
  fi

  if [[ ! -f "${identity}.pub" ]]; then
    echo "âŒ Public key missing â†’ ${identity}.pub"
...
zip_encrypt() {
  ...
  echo "âŒ Need zip or 7z to create password-protected zip." >&2
  return 2
}
...
unzip_decrypt() {
  ...
  echo "âŒ Need unzip or 7z to extract password-protected zip." >&2
  return 2
}
...
tg_send_file() {
  ...
  if [[ -z "$TG_BOT_TOKEN" || -z "$TG_CHAT_ID" ]]; then
    echo "âš ï¸  Telegram not configured. Set TG_BOT_TOKEN and TG_CHAT_ID via: $APP init"
    return 2
  fi
  if ! has_cmd curl; then
    echo "âŒ curl not found. Can't send to Telegram." >&2
    return 2
  fi
  curl -fsSL \
...
cmd_backup() {
  ...
  if tg_send_file "$zipfile" "GSH backup $ts"; then
    ...
    rm -rf ~/gsh-backup-*
    echo "deleted backup file from local storage"
  else
    echo "â„¹ï¸  Not sent to Telegram (configure via: $APP init)"
  fi
}
...
cmd_restore() {
  ...
  if [[ ! -f "$zip_path" ]]; then
    echo "âŒ File not found: $zip_path"
    exit 1
  fi
  ...
  if [[ ! -d "$tmpdir/home/.ssh" && ! -d "$tmpdir/home/bin" ]]; then
    echo "âŒ Backup structure invalid (expected home/.ssh or home/bin inside zip)."
    rm -rf "$tmpdir"
    exit 1
  fi
  ...
  if [[ -d "$HOME/.ssh" ]]; then
    ...
    mv "$HOME/.ssh" "$ssh_bak"
    echo "âœ… Moved current ~/.ssh -> $ssh_bak"
  fi
  if [[ -d "$HOME/bin" ]]; then
    bin_bak="$HOME/bin.bak.$ts"
    mv "$HOME/bin" "$bin_bak"
    echo "âœ… Moved current ~/bin -> $bin_bak"
  fi
